...
class BSTree:
    ...
    def apply(self, f):
        ...
        def subtree_apply(n):
            if n != None:
                n.value = f(n.value)
                subtree_apply(n.left)
                subtree_apply(n.right)
        subtree_apply(self.root)
...
def countSort(someList, m):
    ...
    while i < len(countList):
        r = 0
        while r < countList[i]:
            someOtherList.append(i)
            r += 1
        i += 1
    i = 0
    while i < len(someList):
        someList[i] = someOtherList[i]
        i += 1
